...
public class JokeFetcher {
    ...
    public Jokes getJokes(String jokesToFetch, String timeZone) throws JokeException {
        ...
        List<IJokeFetcher> fetchedJokes = factory.getJokeFetchers(jokesToFetch);
        for (IJokeFetcher fetcher : fetchedJokes) {
            jokes.addJoke(fetcher.getJoke());
        }

        String tzString = dateFormatter.getFormattedDate(new Date(), timeZone);
    ...
    public JokeFetcher(IFetcherFactory factory, IDateFormatter dateFormatter) {
        ...
        Jokes jokes;
        try {
            jokes = this.getJokes("eduprog,chucknorris,chucknorris,moma,tambal", "Europe/Copenhagen");

            jokes.getJokes().forEach((joke) -> {
                System.out.println(joke);
            });
        } catch (JokeException ex) {
            Logger.getLogger(JokeFetcher.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
